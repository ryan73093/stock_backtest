...
#output analysis info(dataframe)

#breakpoint
def break_integrate(self, df, require_inputs):
    df_ = self.integrate(df, require_inputs)
    # define break point
    lst_break = []
    for i in range(len(df_)):
        if df_.iloc[i]['CLOSING_PRICE'] > df_.iloc[:i]['CLOSING_PRICE'].max():
            lst_break.append(1)
        else:
            lst_break.append(0)
    df_['BREAK'] = lst_break
    df_['BREAK_PRICE'] = df_['BREAK'] * df_['CLOSING_PRICE']
    df_['+10%'] = df_['BREAK_PRICE'] * 1.1
    df_['-3%'] = df_['BREAK_PRICE'] * 0.97
    # define sold date and price
    lst_date = []
    lst_sold = []
    for k in range(len(df_)):
        tmpA = df_.iloc[k]['BREAK_PRICE'] * 1.1
        tmpB = df_.iloc[k]['BREAK_PRICE'] * 0.97
        for i, j in zip(df_.iloc[k:]['CLOSING_PRICE'], df_.iloc[k:]['DATE']):
            if (i > tmpA) | (i < tmpB):
                date = j
                sold = i
                break
    ...
    df_['SOLD_DATE'] = lst_date
    df_['SOLD_PRICE'] = lst_sold
    df_['REVENUE'] = df_['SOLD_PRICE'] - df_['BREAK_PRICE']
    return df_

# output revenue(dataframe)
